...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // store universal token + convenience fields used across app
      ...
      const m = err.response?.data?.error || err.response?.data?.message || "Login failed";
      setMsg(m);
    } finally {
      setLoading(false);
    }
    ...
    <section className="min-h-screen bg-gradient-to-br from-indigo-900 via-blue-800 to-indigo-900 flex justify-center items-center px-4">
      <div className="bg-white shadow-2xl rounded-3xl p-10 max-w-md w-full border border-gray-200">
        <div className="text-center mb-6">
          <img src={thaparlogo} alt="logo" className="w-20 mx-auto mb-4 bg-white rounded-full p-2" />
          <h1 className="text-2xl font-bold text-gray-800">Admin Login</h1>
        </div>

        {msg && (
          <div className={`mb-4 p-3 rounded text-center ${msg.toLowerCase().includes("fail") ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
            {msg}
          </div>
        )}
